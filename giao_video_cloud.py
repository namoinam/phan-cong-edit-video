...
def db_read_voice_state() -> dict:
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT name, voice_total, voice_counter FROM products_inventory")
        rows = cursor.fetchall()
        voice_state = {}
        for r in rows:
            voice_state[r[0]] = {
...
def main():
    ...
    # Đọc Supabase Database
    print("\n  🔗 Đọc dữ liệu từ Supabase Cloud...")
    try:
        products_raw = db_read_products()
        print(f"     ✅ products_inventory — {len(products_raw)} SP")
    except Exception as e:
        print(f"     ❌ Lỗi đọc dữ liệu sản phẩm từ Supabase: {e}")
        sys.exit(1)

    try:
        voice_state = db_read_voice_state()
        print(f"     ✅ voice_state — Đọc thành công từ Supabase")
    except Exception as e:
        print(f"     ❌ Lỗi đọc trạng thái voice từ Supabase: {e}")
        sys.exit(1)
    ...
    print("\n  📤 Đang lưu kết quả phân công lên Supabase Cloud...")
    assignments, links = build_history_entry(active_editors, sp_plans)

    conn = get_db_connection()
    if not conn:
        print("     ❌ Kết nối Supabase thất bại khi lưu kết quả!")
        sys.exit(1)
    cursor = conn.cursor()
    try:
        # 1. Lưu assignments và links của ngày hôm nay vào assignments_history
        assignments_payload = {
            "assignments": assignments,
            "links": links
        }
        assignments_json = json.dumps(assignments_payload)
        cursor.execute("""
        INSERT INTO assignments_history (date, remainder_ptr, assignments)
        VALUES (%s, 0, %s)
        ON CONFLICT (date) DO UPDATE SET
            assignments = EXCLUDED.assignments;
        """, (today_str, assignments_json))

        # 2. Cập nhật voice_state cho các sản phẩm trong products_inventory
        for p in sp_plans:
            sp_name = p["name"]
            v_info = voice_state.get(sp_name, {})
            counter = v_info.get("counter", 0)
            total = v_info.get("total", 0)
            cursor.execute(
                "UPDATE products_inventory SET voice_counter = %s, voice_total = %s WHERE name = %s",
                (counter, total, sp_name)
            )
        ...
        conn.rollback()
        print(f"     ❌ Lưu kết quả thất bại: {e}")
        sys.exit(1)
    finally:
        conn.close()
